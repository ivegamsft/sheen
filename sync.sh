...
protect_generated_tokens() {
  ...
  local rules=() removals=()
  for name in sheen.css sheen.js sheen.esm.js sheen.d.ts; do
    path="dist/tokens/$name"
    tracked="$(git -C "$REPO_ROOT" ls-files -v -- "$path")"
    if [ -n "$tracked" ]; then
      # Index flags can hide local edits from diff; never clear them or trust a clean diff.
      while IFS= read -r entry; do
        flags=''
        case "${entry:0:1}" in [a-z]) flags='assume-unchanged' ;; esac
        case "${entry:0:1}" in S|s) flags="${flags}${flags:+, }skip-worktree" ;; esac
        if [ -n "$flags" ]; then
          echo "sheen sync: token cleanup blocked by index flags ($flags): $path. Review the file and index flags before retrying; flags and token outputs were not changed." >&2; return 1
        fi
      done <<< "$tracked"
      git --no-optional-locks -C "$REPO_ROOT" diff --quiet -- "$path" || {
        echo "sheen sync: token cleanup blocked by unstaged changes: $path. Preserve/review your edits before retrying; no token outputs were removed or rebuilt." >&2; return 1;
      }
      git --no-optional-locks -C "$REPO_ROOT" diff --cached --quiet -- "$path" || {
        echo "sheen sync: token cleanup blocked by staged changes: $path. Preserve/review your edits before retrying; no token outputs were removed or rebuilt." >&2; return 1;
      }
    fi
    if [ -L "$REPO_ROOT/$path" ] || { [ -e "$REPO_ROOT/$path" ] && [ ! -f "$REPO_ROOT/$path" ]; }; then
      echo "sheen sync: token cleanup blocked by non-regular output: $path. Resolve the collision before retrying." >&2; return 1
    fi
    if [ -f "$REPO_ROOT/$path" ]; then
      header=''
      IFS= read -r header < "$REPO_ROOT/$path" || true
      header="${header#$'\xef\xbb\xbf'}"
      header="${header%$'\r'}"
      if [ "$name" = sheen.css ]; then
        expected="/* $name — generated by scripts/build-tokens.ps1; do not hand-edit */"
      else
        expected="// $name — generated by scripts/build-tokens.ps1; do not hand-edit"
      fi
      if [ "$header" != "$expected" ]; then
        echo "sheen sync: token cleanup blocked by unrecognized generated ownership: $path. Preserve this file and resolve the collision before retrying; no token outputs were removed or rebuilt." >&2; return 1
      fi
    fi
  ...
  if [ "${#removals[@]}" -gt 0 ]; then
    git -C "$REPO_ROOT" rm --cached -- "${removals[@]}" || {
      echo 'sheen sync: could not remove obsolete generated outputs from version control; inspect the index before retrying.' >&2; return 1;
    }
    echo 'sheen sync: staged deletion of obsolete generated token outputs from version control. Local build artifacts will be regenerated and ignored; commit these deletions with .gitignore.'
  fi
}

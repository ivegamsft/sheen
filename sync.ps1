...
function Protect-GeneratedTokens {
    ...
    foreach ($name in @('sheen.css', 'sheen.js', 'sheen.esm.js', 'sheen.d.ts')) {
        ...
        $tracked = git -C $repoRoot ls-files -v -- $path
        if ($LASTEXITCODE -ne 0) { throw "Cannot inspect tracked token output: $path" }
        if ($tracked) {
            # Index flags can hide local edits from diff; never clear them or trust a clean diff.
            foreach ($entry in @($tracked)) {
                $flags = @()
                if ($entry -cmatch '^[a-z] ') { $flags += 'assume-unchanged' }
                if ($entry -cmatch '^[Ss] ') { $flags += 'skip-worktree' }
                if ($flags.Count -gt 0) {
                    throw "sheen sync: token cleanup blocked by index flags ($($flags -join ', ')): $path. Review the file and index flags before retrying; flags and token outputs were not changed."
                }
            }
            git --no-optional-locks -C $repoRoot diff --quiet -- $path
            if ($LASTEXITCODE -ne 0) { throw "sheen sync: token cleanup blocked by unstaged changes: $path. Preserve/review your edits before retrying; no token outputs were removed or rebuilt." }
            git --no-optional-locks -C $repoRoot diff --cached --quiet -- $path
            if ($LASTEXITCODE -ne 0) { throw "sheen sync: token cleanup blocked by staged changes: $path. Preserve/review your edits before retrying; no token outputs were removed or rebuilt." }
        }
        if (-not $IsWindows) {
            # Test-Path can miss dangling links; FileInfo attributes do not identify Unix FIFOs.
            & /bin/test -L $localPath
            if ($LASTEXITCODE -eq 0) {
                throw "sheen sync: token cleanup blocked by non-regular output: $path. Resolve the collision before retrying."
            }
            if ($LASTEXITCODE -ne 1) { throw "Cannot inspect token output link type: $path" }
        }
        if (Test-Path -LiteralPath $localPath) {
            ...
            if (-not $IsWindows) {
                & /bin/test -f $localPath
                if ($LASTEXITCODE -eq 1) {
                    throw "sheen sync: token cleanup blocked by non-regular output: $path. Resolve the collision before retrying."
                }
                if ($LASTEXITCODE -ne 0) { throw "Cannot inspect token output file type: $path" }
            }
            $reader = [System.IO.File]::OpenText($localPath)
            try { $header = $reader.ReadLine() } finally { $reader.Dispose() }
            $expected = if ($name -eq 'sheen.css') { "/* $name — generated by scripts/build-tokens.ps1; do not hand-edit */" } else { "// $name — generated by scripts/build-tokens.ps1; do not hand-edit" }
            if ($header -cne $expected) {
                throw "sheen sync: token cleanup blocked by unrecognized generated ownership: $path. Preserve this file and resolve the collision before retrying; no token outputs were removed or rebuilt."
            }
        }
        git -C $repoRoot check-ignore --no-index --quiet -- $path
        $ignoreExit = $LASTEXITCODE
        if ($ignoreExit -notin @(0, 1)) { throw "Cannot inspect ignore policy: $path" }
        if ($ignoreExit -eq 1) {
        ...
        if ($tracked) { $removals.Add($path) }
    }
    # Preflight every output before changing ignore rules or the index.
    if ($rules.Count -gt 0) {
        [byte[]]$existing = @()
    ...
    if ($removals.Count -gt 0) {
        git -C $repoRoot rm --cached -- @removals
        if ($LASTEXITCODE -ne 0) { throw 'sheen sync: could not remove obsolete generated outputs from version control; inspect the index before retrying.' }
        Write-Host 'sheen sync: staged deletion of obsolete generated token outputs from version control. Local build artifacts will be regenerated and ignored; commit these deletions with .gitignore.'
    }
}
...
try {
    ...
    if (Test-Path -LiteralPath $upstreamTemplate) {
        if (-not (Test-Path -LiteralPath $sheenSyncWorkflow)) {
            ...
            if ($existingWorkflow.Contains('This file was synced into your repo by basecoat-sheen.') -and
                $existingWorkflow.Contains('source_repo: IBuySpy-Shared/basecoat-sheen')) {
                Copy-Item -LiteralPath $upstreamTemplate -Destination $sheenSyncWorkflow -Force
                Add-ManifestFile -ManifestFiles $manifest.files -RepoRoot $repoRoot -Path $sheenSyncWorkflow
